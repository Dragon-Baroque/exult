...
gint Palette_edit::mouse_press(
	...
	if (event_button_button == 4 || event_button_button == 5) {    // mouse wheel
		return true;
	}
	...
	if (event_button_button == 3) {
		...
		paled->popup_widget = gtk_popover_new_from_model(widget, G_MENU_MODEL(popup));
		g_object_unref(popup);
		if (paled->selected >= 0) {
			GdkRectangle target = {paled->selected_box.x, paled->selected_box.y, paled->selected_box.w, paled->selected_box.h};
			gtk_popover_set_pointing_to(GTK_POPOVER(paled->popup_widget), &target);
		}
...
void Palette_edit::remove_palette() {
	...
	if (EStudio::Prompt("Do you really want to delete the palette you're viewing?", "Yes", "No") != 0) {
		return;
	}
