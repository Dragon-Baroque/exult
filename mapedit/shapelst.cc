...
static void Shape_dropped_here(
	...
	static_cast<Shape_chooser*>(user_data)->shape_dropped_here(file, shape, frame);
}
...
gint Shape_chooser::configure(int width, int height) {
	...
	// Did the size change?
	if (event_configure_width != config_width || event_configure_height != config_height) {
 ...
 */
gint Shape_chooser::mouse_press(GtkGestureClick* click_ctlr, int n_press, double x, double y) {
	GtkWidget* widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(click_ctlr));
	gtk_widget_grab_focus(widget);    // Enables keystrokes.

	guint   event_button_button = gtk_gesture_single_get_current_button(GTK_GESTURE_SINGLE(click_ctlr));
	gdouble event_button_x = x, event_button_y = y;

#ifdef DEBUG
	cout << "Shapes : Clicked to " << event_button_x << " * " << event_button_y << " by " << event_button_button << endl;
#endif
	...
	if (event_button_button == 3) {
		...
		if (selected >= 0) {
			GdkRectangle target
					= {ZoomUp(info[selected].box.x - hoffset), ZoomUp(info[selected].box.y - voffset), ZoomUp(info[selected].box.w),
					   ZoomUp(info[selected].box.h)};
 ...
 */
C_EXPORT gboolean
		on_draw_key_press(GtkEventControllerKey* key_ctlr, guint keyval, guint keycode, GdkModifierType state, gpointer user_data) {
...
time_t Shape_chooser::export_tiled_png(
	...
	for (int f = 0; f < nframes; f++) {
		...
		if (frame->is_rle() || frame->get_width() != c_tilesize || frame->get_height() != c_tilesize) {
			char buf[250];
			snprintf(buf, sizeof(buf), "Can only tile %dx%d flat shapes", c_tilesize, c_tilesize);
			EStudio::Alert("%s", buf);
			return 0;
		}
		int x;
...
void Shape_chooser::edit_shape(
	...
	if (!tiles) {    // One frame?
		...
		if (use_shp) {
			EStudio::Alert("Tiled editing is only supported with PNG format.");
			return;
		}
...
static void Import_png_tiles(
	...
	delete[] oldpal;
	if (w < needw || h < needh) {
		EStudio::Alert("File '%s' image is too small.  %dx%d required.", fname, needw, needh);
		delete[] pixels;
		return;
	}
...
C_EXPORT void on_new_shape_font_toggled(GtkCheckButton* btn, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
...
void Shape_chooser::new_shape() {
	...
	gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(draw), on_new_shape_font_color_draw_expose_event, this, nullptr);
...
GdkContentProvider* Shape_chooser::drag_prepare(
	...
	cout << "Setting selection data (" << shinfo.shapenum << '/' << shinfo.framenum << ')' << endl;
	...
	GdkContentProvider* targets[]
			= {gdk_content_provider_new_for_bytes(U7_TARGET_SHAPEID_NAME, gfile),
			   gdk_content_provider_new_for_bytes(U7_TARGET_DROPTEXT_NAME_MIME, gfile),
...
void Shape_chooser::all_frames_toggled(GtkToggleButton* btn, gpointer user_data) {
	auto*      chooser   = static_cast<Shape_chooser*>(user_data);
...
void Shape_chooser::shp_info_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	static_cast<Shape_chooser*>(user_data)->edit_shape_info();
}

void Shape_chooser::shp_edit_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	static_cast<Shape_chooser*>(user_data)->edit_shape();
}

void Shape_chooser::shp_edtiles_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
...
static void shp_import_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	Create_file_selection(
			"Import frame from a .png file", "<PATCH>", "PNG Images", {"*.png"}, GTK_FILE_CHOOSER_ACTION_OPEN,
			Shape_chooser::import_frame, user_data);
}

static void shp_export_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	Create_file_selection(
			"Export frame to a .png file", "<PATCH>", "PNG Images", {"*.png"}, GTK_FILE_CHOOSER_ACTION_SAVE,
			Shape_chooser::export_frame, user_data);
}

static void shp_export_all_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	Create_file_selection(
			"Choose the base .png file name for all frames", "<PATCH>", nullptr, {}, GTK_FILE_CHOOSER_ACTION_SAVE,
			Shape_chooser::export_all_frames, user_data);
}

static void shp_import_all_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	Create_file_selection(
...
static void shp_export_shape_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	Create_file_selection(
			"Choose the shp file name", "<PATCH>", "Shape files", {"*.shp"}, GTK_FILE_CHOOSER_ACTION_SAVE,
			Shape_chooser::export_shape, user_data);
}

static void shp_import_shape_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	Create_file_selection(
...
static void shp_new_frame_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	static_cast<Shape_chooser*>(user_data)->new_frame();
}

static void shp_new_shape_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
...
static GActionEntry shp_entries[] = {
		{        "info",    Shape_chooser::shp_info_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{        "edit",    Shape_chooser::shp_edit_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{     "edtiles", Shape_chooser::shp_edtiles_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{   "new-frame",              shp_new_frame_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{      "export",                 shp_export_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{      "import",                 shp_import_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{  "export-all",             shp_export_all_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{  "import-all",             shp_import_all_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{"export-shape",           shp_export_shape_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{"import-shape",           shp_import_shape_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{   "new-shape",              shp_new_shape_action, nullptr, nullptr, nullptr, {0, 0, 0}}
};

GMenu* Shape_chooser::create_popup() {
	// Bind popup menu actions.
	GSimpleActionGroup* shp_group = g_simple_action_group_new();
	g_action_map_add_action_entries(G_ACTION_MAP(shp_group), shp_entries, G_N_ELEMENTS(shp_entries), this);
	gtk_widget_insert_action_group(widget_get_top(get_widget()), "shp", G_ACTION_GROUP(shp_group));
	GMenu*       popup = create_popup_internal(true);    // Create popup with groups, files.
...
Shape_chooser::Shape_chooser(
	...
	g_signal_connect(G_OBJECT(draw), "resize", G_CALLBACK(Configure_chooser), this);
	// Set "expose-event" - "draw" handler.
	gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(draw), Shape_chooser::expose, this, nullptr);
	// Keystroke.
	...
	g_signal_connect(G_OBJECT(key_ctlr), "key-pressed", G_CALLBACK(on_draw_key_press), this);
	gtk_widget_set_can_focus(GTK_WIDGET(draw), true);
	// Set mouse click handler.
	GtkEventController* click_ctlr = GTK_EVENT_CONTROLLER(gtk_gesture_click_new());
	gtk_gesture_single_set_button(GTK_GESTURE_SINGLE(click_ctlr), 0);
	gtk_widget_add_controller(GTK_WIDGET(draw), click_ctlr);
	g_signal_connect(G_OBJECT(click_ctlr), "pressed", G_CALLBACK(Mouse_press), this);
	g_signal_connect(G_OBJECT(click_ctlr), "released", G_CALLBACK(Mouse_release), this);
	// Mouse motion.
	GtkEventController* drag_source = GTK_EVENT_CONTROLLER(gtk_drag_source_new());
	g_signal_connect(G_OBJECT(drag_source), "prepare", G_CALLBACK(drag_prepare), this);
	...
	GtkAdjustment* shape_adj = GTK_ADJUSTMENT(gtk_adjustment_new(0, 0, std::ceil(std::max(get_count(), 1) / 4.0), 1, 1, 1));
