...
static inline void menu_add_action(GMenu* menu, const char* label, const char* action) {
	GMenuItem* item = g_menu_item_new(label, action);
	g_menu_append_item(menu, item);
	g_object_unref(item);
}

static inline void menu_add_section(GMenu* menu, const char* label, GMenu* section) {
...
static inline void menu_add_submenu(GMenu* menu, const char* label, GMenu* submenu) {
...
public:
	...
	// Preferences.
	static void on_prefs_background_expose_event(GtkDrawingArea* widget, cairo_t* cairo, int x, int y, gpointer user_data);
...
bool Get_flag_cbox(
		gpointer widget, unsigned long* oflags, unsigned long* xflags, unsigned long* type_flags, GtkCheckButton*& cbox,
