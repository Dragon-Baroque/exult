...
C_EXPORT void on_npc_show_gump_clicked(GtkButton* btn, gpointer user_data) {
	...
	auto           addr = reinterpret_cast<uintptr>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(btn))), "user_data"));
 ...
 */
C_EXPORT gboolean on_npc_window_delete_event(GtkWidget* widget, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
	ExultStudio* studio = ExultStudio::get_instance();
...
C_EXPORT void on_npc_usecode_browse_clicked(GtkButton* button, gpointer user_data) {
	...
	ExultStudio* studio = ExultStudio::get_instance();
	const char*  uc     = studio->browse_usecode(true, studio->get_widget("npc_window"));
	if (*uc) {
...
void ExultStudio::open_npc_window(
	...
	if (data) {
		...
		gtk_widget_set_hexpand(get_widget("npc_apply_btn"), false);
		...
		gtk_widget_set_hexpand(get_widget("npc_apply_btn"), true);
		gtk_widget_set_visible(get_widget("npc_status"), false);
	}
...
void ExultStudio::init_new_npc() {
	...
	// Set flag checkboxes.
	for (GtkWidget* child = gtk_widget_get_first_child(ftable); child; child = gtk_widget_get_next_sibling(child)) {
	...
	for (GtkWidget* child = gtk_widget_get_first_child(ptable); child; child = gtk_widget_get_next_sibling(child)) {
		GtkSpinButton* spin;
		int            pnum;
...
int ExultStudio::init_npc_window(unsigned char* data, int datalen) {
	...
	for (GtkWidget* child = gtk_widget_get_first_child(ftable); child; child = gtk_widget_get_next_sibling(child)) {
		...
		unsigned long*  bits;
		int             fnum;
		if (Get_flag_cbox(child, &oflags, &xflags, &type_flags, cbox, bits, fnum)) {
			gtk_check_button_set_active(GTK_CHECK_BUTTON(cbox), (*bits & (1 << fnum)) != 0);
		}
	}
	// Set properties.
	GtkContainer* ptable = GTK_CONTAINER(get_widget("npc_props_table"));
...
static void Set_sched_btn(GtkWidget* btn, gpointer user_data) {
	g_signal_connect(G_OBJECT(btn), "clicked", G_CALLBACK(ExultStudio::schedule_btn_clicked), user_data);
}
...
C_EXPORT void on_npc_set_sched(
	...
	if (first) {    // First time?  Set handlers.
		first = 0;
		for (GtkWidget* child = gtk_widget_get_first_child(btns); child; child = gtk_widget_get_next_sibling(child)) {
			Set_sched_btn(child, schedwin);
		}
	}
	// Store label as dialog's data.
	g_object_set_data(G_OBJECT(schedwin), "user_data", label);
	gtk_window_set_transient_for(GTK_WINDOW(schedwin), GTK_WINDOW(studio->get_widget("npc_window")));
