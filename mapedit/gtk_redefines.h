...
static inline void gdk_cairo_get_clip_rectangle(cairo_t* cr, GdkRectangle* rect) {
	double x1, y1, x2, y2;
...
#	define gtk_menu_bar_new_from_model(m) gtk_popover_menu_bar_new_from_model((m))

static inline GtkWidget* gtk_popover_new_from_model(GtkWidget* p, GMenuModel* m) {
	GtkWidget* r = gtk_popover_menu_new_from_model_full(m, GTK_POPOVER_MENU_NESTED);
...
#	define gtk_box_reorder_child(b, c, n) gtk_box_reorder_child_after((b), (c), nullptr)
...
#	define gtk_entry_set_text(w, t) gtk_editable_set_text(GTK_EDITABLE((w)), (t))

static inline bool gtk_css_provider_load_from_path(GtkCssProvider* p, const char* s, GError** e) {
...
static inline void gtk_box_pack_start(GtkBox* b, GtkWidget* w, gboolean e, gboolean f, guint p) {
	gtk_box_append(b, w);
	if (gtk_orientable_get_orientation(GTK_ORIENTABLE(b)) == GTK_ORIENTATION_HORIZONTAL) {
