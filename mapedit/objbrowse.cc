...
void Object_browser::obj_group_add_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	...
	Shape_group_file* groups  = chooser->group ? chooser->group->get_file() : ExultStudio::get_instance()->get_cur_groups();
...
void Object_browser::add_group_submenu(GMenu* popup) {
	...
	if (gcnt > 1 ||    // Groups besides ours?
		...
		for (int i = 0; i < gcnt; i++) {
			...
			char action[128];
			snprintf(action, 128, "obj.group-add('%s')", grp->get_name());
...
void Create_file_selection(
	...
	img = gtk_image_new_from_icon_name((action == GTK_FILE_CHOOSER_ACTION_OPEN) ? "document-open" : "document-save");
	gtk_button_set_image(GTK_BUTTON(btn), img);
	...
	if (path != nullptr && is_system_path_defined(path)) {
		...
		gtk_file_chooser_set_current_folder(fsel, g_file_new_for_path(startdir.c_str()), nullptr);
	}
	if (!filters.empty()) {
...
void Object_browser::obj_file_save_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
...
void Object_browser::obj_file_revert_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	auto* chooser = static_cast<Object_browser*>(user_data);
	if (!chooser->file_info) {
		return;    // No file?
...
static GActionEntry obj_entries[] = {
		{  "group-add",   Object_browser::obj_group_add_action,     "s", nullptr, nullptr, {0, 0, 0}},
		{  "file-save",   Object_browser::obj_file_save_action, nullptr, nullptr, nullptr, {0, 0, 0}},
		{"file-revert", Object_browser::obj_file_revert_action, nullptr, nullptr, nullptr, {0, 0, 0}}
};
...
GMenu* Object_browser::create_popup_internal(bool files    // Include 'files'.
	...
	g_action_map_add_action_entries(G_ACTION_MAP(obj_group), obj_entries, G_N_ELEMENTS(obj_entries), this);
	gtk_widget_insert_action_group(widget_get_top(get_widget()), "obj", G_ACTION_GROUP(obj_group));
	GMenu* popup = g_menu_new();    // Create popup menu.
	if (files) {
		// Add "File" submenu.
		GMenu* menu = g_menu_new();
		menu_add_action(menu, "Save", "obj.file-save");
		menu_add_action(menu, "Revert", "obj.file-revert");
...
GtkWidget* Object_browser::create_controls(int controls    // Browser_control flags.
	...
	if (controls & static_cast<int>(find_controls)) {
		...
		GtkWidget* find_down = EStudio::Create_arrow_button(GTK_ARROW_DOWN, G_CALLBACK(on_find_down), this);
		...
		GtkWidget* find_up = EStudio::Create_arrow_button(GTK_ARROW_UP, G_CALLBACK(on_find_up), this);
		widget_set_margins(find_up, 1 * HMARGIN, 2 * HMARGIN, 2 * VMARGIN, 2 * VMARGIN);
		gtk_box_pack_start(GTK_BOX(hbox3), find_up, true, true, 0);

		g_signal_connect(G_OBJECT(find_text), "activate", G_CALLBACK(on_find_key), this);
	}
	/*
	 *  The 'Locate' controls.
	 */
	if (controls & static_cast<int>(locate_controls)) {
		...
		loc_down = EStudio::Create_arrow_button(GTK_ARROW_DOWN, G_CALLBACK(on_loc_down), this);
		widget_set_margins(loc_down, 2 * HMARGIN, 1 * HMARGIN, 2 * VMARGIN, 2 * VMARGIN);
		gtk_box_pack_start(GTK_BOX(bbox), loc_down, true, true, 0);

		loc_up = EStudio::Create_arrow_button(GTK_ARROW_UP, G_CALLBACK(on_loc_up), this);
	...
	if (controls & static_cast<int>(move_controls)) {
		...
		move_down = EStudio::Create_arrow_button(GTK_ARROW_DOWN, G_CALLBACK(on_move_down), this);
		...
		move_up = EStudio::Create_arrow_button(GTK_ARROW_UP, G_CALLBACK(on_move_up), this);
...
void Object_browser::draw_vscrolled(       // For scroll events.
	...
	GtkAdjustment* adj       = gtk_scrollbar_get_adjustment(GTK_SCROLLBAR(browser->vscroll));
...
void Object_browser::enable_draw_vscroll(GtkWidget* draw) {
	GtkEventController* vscroll_ctlr = GTK_EVENT_CONTROLLER(gtk_event_controller_scroll_new(GTK_EVENT_CONTROLLER_SCROLL_VERTICAL));
