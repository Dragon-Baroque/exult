...
void Object_browser::obj_group_add_action(
...
void Object_browser::obj_file_save_action(
		GSimpleAction* action, GVariant* parameter, gpointer user_data) {
...
void Object_browser::obj_file_revert_action(
		GSimpleAction* action, GVariant* parameter, gpointer user_data) {
	ignore_unused_variable_warning(action, parameter);
	auto* chooser = static_cast<Object_browser*>(user_data);
	if (!chooser->file_info) {
...
static GActionEntry obj_entries[] = {
		{  "group-add",
		 Object_browser::obj_group_add_action,
		 "s", nullptr,
		 nullptr, {0, 0, 0}},
		{  "file-save",
		 Object_browser::obj_file_save_action,
		 nullptr, nullptr,
		 nullptr, {0, 0, 0}},
		{"file-revert",
		 Object_browser::obj_file_revert_action,
		 nullptr, nullptr,
		 nullptr, {0, 0, 0}}
};
