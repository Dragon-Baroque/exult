...
C_EXPORT gboolean on_groups_new_name_key_press(GtkEntry* entry, gpointer user_data) {
...
void ExultStudio::setup_groups() {
	...
	if (!oldmod) {    // Create model first time.
		...
		g_signal_connect(G_OBJECT(get_widget("groups_new_name")), "activate", G_CALLBACK(on_groups_new_name_key_press), this);
	} else {
...
void ExultStudio::export_group() {
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		char* filename = g_file_get_path(gtk_file_chooser_get_file(GTK_FILE_CHOOSER(dialog)));
		...
		try {
			...
			EStudio::Alert("Group '%s' exported successfully.", grp->get_name());
 ...
 */
C_EXPORT gboolean on_group_window_delete_event(GtkWidget* widget, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
...
C_EXPORT void on_group_up_clicked(GtkToggleButton* button, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
	auto*        chooser = static_cast<Shape_chooser*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(button))), "browser"));
...
C_EXPORT void on_group_shape_remove_clicked(GtkToggleButton* button, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
	auto*        chooser = static_cast<Object_browser*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(button))), "browser"));
	Shape_group* grp     = chooser->get_group();
	const int    i       = chooser->get_selected();
	if (grp && i >= 0) {
...
void ExultStudio::open_group_window(Shape_group* grp) {
	...
	g_signal_connect(G_OBJECT(grpwin), "close-request", G_CALLBACK(on_group_window_delete_event), this);
...
C_EXPORT void on_group_okay_clicked(GtkButton* button, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
	GtkWidget* grpwin = widget_get_top(GTK_WIDGET(button));
