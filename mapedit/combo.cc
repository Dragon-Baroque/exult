 ...
 */
void Combo_editor::on_combo_draw_expose_event(
		GtkDrawingArea* widget,    // The view window.
		cairo_t* cairo, int width, int height, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
	auto* combo = static_cast<Combo_editor*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(widget))), "user_data"));
...
C_EXPORT void on_combo_remove_clicked(GtkButton* button, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
	auto* combo = static_cast<Combo_editor*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(button))), "user_data"));
	combo->remove();
}
...
C_EXPORT void on_combo_name_changed(GtkEntry* entry, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
	auto* combo = static_cast<Combo_editor*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(entry))), "user_data"));
	if (combo) {
 ...
 */
C_EXPORT gboolean on_combo_window_delete_event(GtkWidget* widget, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
 ...
 */
C_EXPORT gboolean
		on_combo_draw_button_press_event(GtkGestureClick* click_ctlr, int n_press, double x, double y, gpointer user_data) {
	ignore_unused_variable_warning(user_data);

	GtkWidget* widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(click_ctlr));
	auto*      combo  = static_cast<Combo_editor*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(widget))), "user_data"));
	return combo->mouse_press(click_ctlr, n_press, x, y);
}
...
Combo_editor::Combo_editor(
	...
	g_signal_connect(G_OBJECT(click_ctlr), "pressed", G_CALLBACK(on_combo_draw_button_press_event), nullptr);
	gtk_widget_set_can_focus(GTK_WIDGET(draw), true);
	gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(draw), on_combo_draw_expose_event, this, nullptr);
	if (first) {    // Indicate the events we want.
		g_signal_connect(G_OBJECT(win), "close-request", G_CALLBACK(on_combo_window_delete_event), nullptr);
		first = false;
	}
	set_controls();
}
...
gint Combo_editor::mouse_press(GtkGestureClick* click_ctlr, int n_press, double x, double y) {
...
GdkContentProvider* Combo_chooser::drag_prepare(
	...
	int      len  = Store_u7_comboid(buf, foot.w, foot.h, foot.x + foot.w - 1 - hot->tx, foot.y + foot.h - 1 - hot->ty, cnt, ents);
	assert(len <= buflen);
	const char*         target = reinterpret_cast<const char*>(buf);
	GBytes*             gfile  = g_bytes_new(target, len);
	GdkContentProvider* targets[]
			= {gdk_content_provider_new_for_bytes(U7_TARGET_COMBOID_NAME, gfile),
...
void Combo_chooser::drag_begin(
	...
	if (shape) {
		chooser->set_drag_icon(drag, shape);
	}
	return;
}
...
static gint Mouse_press(GtkGestureClick* click_ctlr, int n_press, double x, double y, gpointer user_data);
static gint Mouse_release(GtkGestureClick* click_ctlr, int n_press, double x, double y, gpointer user_data);
static gint Configure_chooser(GtkWidget* widget, int width, int height, gpointer user_data);
...
Combo_chooser::Combo_chooser(
	...
	gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(draw), expose, this, nullptr);
	// Keystroke.
	...
	g_signal_connect(G_OBJECT(key_ctlr), "key-pressed", G_CALLBACK(on_combo_key_press), this);
	gtk_widget_set_can_focus(GTK_WIDGET(draw), true);
...
gint Combo_chooser::configure(int width, int height) {
...
gint Combo_chooser::mouse_press(GtkGestureClick* click_ctlr, int n_press, double x, double y) {
	GtkWidget* widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(click_ctlr));
...
#ifdef DEBUG
	cout << "Combos : Clicked to " << event_button_x << " * " << event_button_y << " by " << event_button_button << endl;
#endif
	...
	const int      absx   = ZoomDown(static_cast<int>(event_button_x));
	...
	if (i == info.size() && event_button_button == 1) {
	...
	if (event_button_button == 3) {
		...
		if (selected >= 0) {
			GdkRectangle target
					= {ZoomUp(info[selected].box.x), ZoomUp(info[selected].box.y), ZoomUp(info[selected].box.w),
