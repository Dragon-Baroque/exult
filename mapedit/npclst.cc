 ...
 */
gint Npc_chooser::mouse_press(GtkGestureClick* click_ctlr, int n_press, double x, double y) {
...
#ifdef DEBUG
	cout << "Npcs : Clicked to " << event_button_x << " * " << event_button_y << " by " << event_button_button << endl;
#endif
...
gboolean Npc_chooser::drag_data_received(GtkDropTarget* dest, GValue* value, double x, double y, gpointer user_data) {
...
void npc_edit_action(GSimpleAction* action, GVariant* parameter, gpointer user_data) {
...
static GActionEntry npc_entries[] = {
		{"edit", npc_edit_action, nullptr, nullptr, nullptr, {0, 0, 0}}
};

GMenu* Npc_chooser::create_popup() {
	// Bind popup menu actions.
	GSimpleActionGroup* npc_group = g_simple_action_group_new();
	g_action_map_add_action_entries(G_ACTION_MAP(npc_group), npc_entries, G_N_ELEMENTS(npc_entries), this);
	gtk_widget_insert_action_group(widget_get_top(get_widget()), "npc", G_ACTION_GROUP(npc_group));
	// Create popup with groups, but not files.
	GMenu* popup = create_popup_internal(false);
	if (selected >= 0) {    // Add editing choices.
		menu_add_action(popup, "Edit...", "npc.edit");
	}
	return popup;
}
...
Npc_chooser::Npc_chooser(
	...
	g_signal_connect(G_OBJECT(key_ctlr), "key-pressed", G_CALLBACK(on_npc_draw_key_press), this);
