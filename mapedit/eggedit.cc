 ...
 */
C_EXPORT gboolean on_egg_window_delete_event(GtkWidget* widget, gpointer user_data) {
...
C_EXPORT void on_egg_browse_usecode_clicked(GtkButton* button, gpointer user_data) {
	...
	ExultStudio* studio = ExultStudio::get_instance();
	const char*  uc     = studio->browse_usecode(true, studio->get_widget("egg_window"));
	if (*uc) {
 ...
 */
C_EXPORT void on_teleport_coord_toggled(GtkCheckButton* btn, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
	ExultStudio* studio = ExultStudio::get_instance();
