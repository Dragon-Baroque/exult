...
Shape_gump_single::Shape_gump_single(
	...
	gtk_drawing_area_set_draw_func(
			GTK_DRAWING_AREA(draw), Shape_gump_single::on_draw_expose_event,
			this, nullptr);
...
void Shape_gump_single::on_draw_expose_event(
		GtkDrawingArea* widget, cairo_t* cairo, int width, int height,
		gpointer user_data) {
	ignore_unused_variable_warning(widget);
	auto*        single = static_cast<Shape_gump_single*>(user_data);
	...
	if (shnum >= 0
		&& gtk_check_button_get_active(
				GTK_CHECK_BUTTON(single->show_checkmark_widget))
	...
	if (gtk_check_button_get_active(
				GTK_CHECK_BUTTON(single->show_container_widget))
