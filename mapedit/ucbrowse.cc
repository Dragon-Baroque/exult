...
C_EXPORT void on_usecodes_ok_clicked(GtkButton* btn, gpointer user_data) {
	...
	auto* ucb = static_cast<Usecode_browser*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(btn))), "user_data"));
	ucb->okay();
}
...
C_EXPORT void on_usecodes_treeview_row_activated(
	...
	auto* ucb = static_cast<Usecode_browser*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(treeview))), "user_data"));
	ucb->okay();
}
 ...
 */
C_EXPORT gboolean on_usecodes_dialog_delete_event(GtkWidget* widget, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
...
Usecode_browser::Usecode_browser() {
	...
	gtk_window_set_transient_for(GTK_WINDOW(win), GTK_WINDOW(ExultStudio::get_instance()->get_widget("main_window")));
