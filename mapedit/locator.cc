...
C_EXPORT void on_loc_close_clicked(GtkButton* btn, gpointer user_data) {
	...
	auto* loc = static_cast<Locator*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(btn))), "user_data"));
	loc->show(false);
}
 ...
 */
C_EXPORT gboolean on_loc_window_delete_event(GtkWidget* widget, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
...
C_EXPORT gboolean on_loc_draw_configure_event(
	...
	auto* loc = static_cast<Locator*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(widget))), "user_data"));
 ...
 */
C_EXPORT gboolean on_loc_draw_button_press_event(GtkGestureClick* click_ctlr, int n_press, double x, double y, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
...
C_EXPORT gboolean
		on_loc_draw_button_release_event(GtkGestureClick* click_ctlr, int n_press, double x, double y, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
	GtkWidget* widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(click_ctlr));
	auto*      loc    = static_cast<Locator*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(widget))), "user_data"));
...
C_EXPORT gboolean on_loc_draw_motion_notify_event(GtkEventController* motion_ctlr, double x, double y, gpointer user_data) {
	ignore_unused_variable_warning(user_data);
	GtkWidget* widget = gtk_event_controller_get_widget(GTK_EVENT_CONTROLLER(motion_ctlr));
	auto*      loc    = static_cast<Locator*>(g_object_get_data(G_OBJECT(widget_get_top(GTK_WIDGET(widget))), "user_data"));
...
Locator::Locator() {
	...
	GtkEventController* motion_ctlr = GTK_EVENT_CONTROLLER(gtk_event_controller_motion_new());
	gtk_widget_add_controller(GTK_WIDGET(draw), motion_ctlr);
	g_signal_connect(G_OBJECT(motion_ctlr), "motion", G_CALLBACK(on_loc_draw_motion_notify_event), this);
	...
	g_signal_connect(G_OBJECT(click_ctlr), "pressed", G_CALLBACK(on_loc_draw_button_press_event), this);
	g_signal_connect(G_OBJECT(click_ctlr), "released", G_CALLBACK(on_loc_draw_button_release_event), this);
	g_signal_connect(G_OBJECT(draw), "resize", G_CALLBACK(on_loc_draw_configure_event), nullptr);
	gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(draw), Locator::on_loc_draw_expose_event, this, nullptr);
...
gboolean Locator::mouse_press(GtkGestureSingle* gesture, int n_press, double x, double y) {
	dragging                    = false;
...
gboolean Locator::mouse_release(GtkGestureSingle* gesture, int n_press, double x, double y) {
...
gboolean Locator::mouse_motion(GtkEventController* event_ctlr, double x, double y) {
